...
Op de zaakpagina (`/zaken/[id]`) staat één algemeen upload-blok ("Documenten uploaden", multi-select) — geen los knopje per checklist-item meer, want er is geen vooraf berekende checklist meer. De uitleg over welke bestandstypen/herkenning ondersteund worden staat niet meer standaard uitgeschreven op het scherm, maar achter een "i"-icoontje (`src/components/InfoTooltip.tsx`, hover of klik) om het formulier compact te houden.

**Upload gaat rechtstreeks van de browser naar Supabase Storage** (`src/lib/documenten/client-upload.ts`, aangeroepen vanuit de client-componenten `upload-documenten-form.tsx` en `genereer-rapport-form.tsx`) — bewust NIET via de request-body van een server action. Vercel's serverless functions accepteren maximaal 4,5MB per requestbody, een harde platformlimiet die los staat van Next.js' eigen `serverActions.bodySizeLimit` (zie "Setup"/`next.config.ts`) en die bij een batch van meerdere gescande PDF's al snel overschreden wordt — dat gaf eerst een `Body exceeded 1MB limit`-fout (vóór het ophogen van `bodySizeLimit`) en daarna een rauwe `Unexpected end of form`-fout (ná het ophogen — Next.js' eigen check was ruimer, maar Vercel's infrastructuur knipte de verbinding alsnog af). Pas ná een geslaagde upload naar Storage stuurt de pagina alleen het (kleine) storage-pad naar de server. Elk zo geüpload bestand doorloopt daar `src/lib/documenten/verwerk-upload.ts` (`verwerkGeuploadBestand`):

1. **Download** vanuit Supabase Storage (bucket `documenten`) om de inhoud te kunnen lezen.
...
3. **Metadata-aanvulling**: dezelfde aanroep extraheert ook `ondernemingNaam`, `rechtsvorm`, `oprichtingsdatum`, `kvkNummer`, `ongevalsdatum` en de verzekeraar-/belangenbehartigergegevens (naam/bureau, contactpersoon, e-mail, kenmerk — meestal te vinden in de opdrachtbrief) als het document die vermeldt (alleen wat er echt in staat, nooit gegokt). Al deze zaakvelden worden alleen ingevuld als ze nog leeg zijn; een onderneming wordt gematcht op naam (case-insensitive) — bij een match worden alleen de nog lege velden aangevuld, bij geen match wordt een nieuwe onderneming aangemaakt. Bestaande, al ingevulde waarden worden nooit overschreven.
...
**Linkerzijbalk**: bovenaan het "Zaakgegevens-vierkant" — wat er over de zaak bekend is (naam betrokkene, per onderneming naam/oprichtingsdatum/KvK-nummer, ongevalsdatum, verzekeraar, belangenbehartiger, aanmaakdatum; ontbrekende velden tonen "onbekend"), met onderaan een **"Zaak verwijderen"**-knop (bevestigingsdialoog via `window.confirm`, vermeldt expliciet dat ook alle ondernemingen/documenten/rapportages verdwijnen — onomkeerbaar; Postgres-cascade ruimt de gekoppelde rijen op, daarna worden de Storage-bestanden verwijderd, redirect naar het dashboard). Direct daaronder, in dezelfde zijbalk, het **"Documenten uploaden"**-blok (`upload-documenten-form.tsx`).
...
**Aanvullende informatie (2026-08-18):** op de zaakpagina staat een sectie "Aanvullende informatie" (tabel `zaak_notities`) met losse tekstblokken die **bij elke toekomstige generatie** voor de zaak worden meegegeven, totdat ze bewust gewijzigd of verwijderd worden. Elk blok is los te bewerken (`notitie-blok.tsx`, toggle tussen weergave en een bewerk-tekstvak) of te verwijderen; lange blokken (bv. een geplakte opdrachtbrief) tonen standaard een compacte, ingeklapte eenregelige preview met een pijltje om uit te klappen. `genereerRapportageActie` haalt alle notities van de zaak op en geeft ze als aparte, duidelijk gelabelde sectie mee aan de prompt (`genereer.ts`) — dit verving zowel het oude losse tekstvak "Extra informatie" (gold maar voor één versie) als het losse bestandsupload-veld op het genereer-formulier zelf (dat dekte toch al af wat "Documenten uploaden" al deed: alles wat daar geüpload wordt, telt sowieso mee bij de eerstvolgende generatie). `rapportages.extra_context` bestaat als kolom nog wel — de versiepagina kan een versie nog steeds achteraf van eigen context voorzien (zie "Versies" hieronder).

**"Genereer rapport"** (`genereer-rapport-form.tsx`) is nu alleen nog een knop — in de groene logokleur (`#12756A`) — die naast de "Rapportages (N)"-link in de paginakop staat, net zo compact gepresenteerd, in plaats van een los blok met eigen kaart.
...
3. `src/lib/rapportage/genereer.ts` bouwt een prompt met die gegevens plus het structuursjabloon in `src/lib/rapportage/sjabloon.ts` — de exacte lay-out (omslag-/gegevenstabellen, hoofdstukvolgorde, ondertekening, bijlagenlijst) is primair afgeleid van het kantoor-eigen blanco formatdocument ("Format BEDRIJFSKUNDIGE RAPPORTAGE" — een placeholder-sjabloon zonder cliëntgegevens, de gezaghebbende bron), aangevuld met twee echte voorbeeldrapportages (een BV met holdingstructuur, en een eenmanszaak met een apart onderbouwd gemist project); de brondocumenten zijn na het afleiden van de structuur niet bewaard, alleen de generieke opbouw staat in de code. Hoofdstuk 3 en 4 bevatten naast de winst-en-verliesrekening ook een balansanalyse (solvabiliteit en current ratio, met de standaardformules/-normen en -voetnoten uit het format) — als er geen balansgegevens zijn aangeleverd, benoemt Claude dat expliciet in plaats van de kengetallen te verzinnen of de subkop over te slaan. De twee voorbeeldrapportages bleken structureel te verschillen op één punt: of de would-be-reconstructie een eigen hoofdstuk krijgt. Het sjabloon laat Claude dat zelf beoordelen op basis van de complexiteit van de zaak — een eigen hoofdstuk bij een omvangrijke, apart te onderbouwen would-be-berekening (bv. een specifiek gemist project), anders blijft het onderdeel van "Beantwoording vragen".
4. Claude (`claude-opus-5`, streaming, adaptive thinking) schrijft een concept in markdown, met aannames expliciet gemarkeerd als `[AANNAME]` in plaats van verzonnen zekerheden. Er is geen vaste checklist meer om tegen af te zetten — het laatste hoofdstuk (Voortgang, hoofdstuk 6 of 7 afhankelijk van de would-be-hoofdstukkeuze) laat Claude op basis van vakkundig oordeel benoemen wat er nog ontbreekt.
...
**Lettertype, kopstijlen en omslagblok (2026-08-18):** een tweede opmaakronde, nadat bleek dat het lettertype/de kopkleuren niet klopten en het omslagblok nog als een echte tabel met "Label"/"Waarde"-koppen werd weergegeven (was nooit de bedoeling — het origineel gebruikt géén tabel met kolomkoppen). Uitgezocht door het blanco formatdocument nogmaals uit te pakken en `word/styles.xml` + de directe run-opmaak in `word/document.xml` te lezen:
- Lettertype door het hele document: **Verdana** (niet Arial — dat is alleen de in-app-preview), 10pt body-tekst.
- Hoofdstukkoppen (markdown `##`, Word-stijl "Kop1" in het origineel): Verdana, vet, kleur `#702372` (paars/bordeaux).
- Genummerde subparagrafen 1.1/1.2/1.3 (markdown `###`, Word-stijl "Kop2"): Verdana, kleur `#702372`, NIET vet.
- Overige subkopjes ("Winst- en verliesrekening", "Balans", "Solvabiliteit", ondernemingsnamen in hoofdstuk 2, etc.) zijn in het origineel géén Word-kopstijl, gewoon vetgedrukte platte tekst — `sjabloon.ts` instrueert dit nu expliciet (**vetgedrukt**, geen markdown-kop) en `naar-docx.ts` renderde ze voorheen ten onrechte als een aparte kopstijl.
- Het omslagblok (`src/lib/rapportage/sjabloon.ts`) is geen markdown-tabel meer maar een vetgedrukte paarse kopregel + platte "Label: waarde"-regels; `naar-docx.ts` herkent deze vier vaste kopregels (`isOmslagblokKop`) en verpakt ze alsnog in een kader met gestippelde paarse rand (`omslagblokTabel`) — exact zoals het origineel (een tabel met één cel, geen kolomkoppen, `w:tblBorders` met `dotted`/`702372`), maar dan zonder dat Claude zelf een markdown-tabelsyntax hoeft te genereren.
- Geverifieerd door zowel een synthetische testmarkdown als een echte gegenereerde rapportage lokaal (buiten de browser om) door `naar-docx.ts` te halen en de resulterende Word-XML te controleren op stijldefinities, kaderranden en de afwezigheid van "Label"/"Waarde"-tekst.
...
- ~~Uploadfout bij meerdere/grote bestanden~~ — **Resolved 2026-08-13:** upload gaat sinds deze fix rechtstreeks van de browser naar Supabase Storage in plaats van via een server-action-body, en omzeilt daarmee zowel Next.js' eigen `serverActions.bodySizeLimit` (nog wel opgehoogd naar 50MB in `next.config.ts`, als vangnet voor andere server actions) als Vercel's harde, niet-instelbare platformlimiet van 4,5MB per requestbody — die laatste bleek de echte oorzaak van een eerdere `Unexpected end of form`-fout bij het uploaden van meerdere gescande PDF's. Zie "Zaken en documentclassificatie" hierboven voor de volledige uitleg.
