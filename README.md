...
3. `src/lib/rapportage/genereer.ts` bouwt een prompt met die gegevens plus het structuursjabloon in `src/lib/rapportage/sjabloon.ts` — de exacte lay-out (omslag-/gegevenstabellen, hoofdstukvolgorde, ondertekening, bijlagenlijst) is afgeleid van twee echte voorbeeldrapportages (een BV met holdingstructuur, en een eenmanszaak met een apart onderbouwd gemist project); de brondocumenten zijn na het afleiden van de structuur niet bewaard, alleen de generieke opbouw staat in de code. De twee voorbeelden bleken structureel te verschillen op één punt: of de would-be-reconstructie een eigen hoofdstuk krijgt. Het sjabloon laat Claude dat zelf beoordelen op basis van de complexiteit van de zaak — een eigen hoofdstuk bij een omvangrijke, apart te onderbouwen would-be-berekening (bv. een specifiek gemist project), anders blijft het onderdeel van "Beantwoording vragen".
