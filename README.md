...
- Geen limiet/waarschuwing bij zeer grote of zeer veel documenten (Claude API-limiet: 32 MB per request, 600 pagina's). Wel is de generieke Next.js Server Actions-limiet (standaard 1MB per requestbody, blokkeerde het uploaden van meerdere gescande PDF's tegelijk) opgehoogd naar 50MB in `next.config.ts` — dat is de totale batch-uploadgrootte, niet een per-document-limiet.
