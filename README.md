...
**Aanvullende informatie (2026-08-18):** op de zaakpagina staat een sectie "Aanvullende informatie" (tabel `zaak_notities`) met losse tekstblokken die **bij elke toekomstige generatie** voor de zaak worden meegegeven, totdat ze bewust gewijzigd of verwijderd worden. Elk blok is los te bewerken (`notitie-blok.tsx`, toggle tussen weergave en een bewerk-tekstvak) of te verwijderen; lange blokken (bv. een geplakte opdrachtbrief) tonen standaard een compacte, ingeklapte eenregelige preview met een pijltje om uit te klappen. `genereerRapportageActie` haalt alle notities van de zaak op en geeft ze als aparte, duidelijk gelabelde sectie mee aan de prompt (`genereer.ts`).

Het losse tekstvak "Extra informatie" op het genereer-formulier (dat alleen voor één versie zou gelden) is inmiddels weer verwijderd — de persistente notities hierboven vervangen die behoefte. Het genereer-formulier zelf (`genereer-rapport-form.tsx`) staat niet meer in de rechterzijbalk maar prominent, gecentreerd, in de hoofdkolom vlak onder de paginatitel (bij de "Rapportages"-link). Onderliggend bestaat `rapportages.extra_context` nog wel — de versiepagina kan een versie nog steeds achteraf van eigen context voorzien (zie "Versies" hieronder) — alleen het losse invoerveld bij het genereren zelf is vervallen.

Het bestandsupload-veld voor "extra documenten bij deze generatie" is ook uit dit formulier gehaald — dat dekte al af wat het aparte "Documenten uploaden"-blok (rechterzijbalk) al deed: alles wat daar geüpload wordt, telt sowieso mee bij de eerstvolgende generatie. Het genereer-formulier bestaat nu dus alleen nog uit de knop zelf.
